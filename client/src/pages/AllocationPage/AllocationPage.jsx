...
export default function AllocationPage() {
  ...
  const fetchAllocations = useCallback(async () => {
    ...
    try {
      ...
      setAllocationsError(
        err.response?.data?.message || "Could not load allocation list."
  ...
  const handleAllocate = async () => {
    if (
      ...
      setAllocationMessage({
        type: "error",
        text: "Please select Student(s), Tutor, and Course Duration.",
      }); // Thông báo lỗi tiếng Anh
      return;
    }
  ...
  const handleDeactivate = async (allocationId) => {
    if (
      !window.confirm("Are you sure you want to deactivate this allocation?")
    ) {
      return;
    }
    setAllocationMessage({ type: "", text: "" });
    try {
      ...
      setAllocationMessage({
        type: "success",
        text: "Allocation deactivated successfully!",
      }); // Thông báo lỗi tiếng Anh
      fetchAllocations();
    } catch (err) {
  ...
  return (
    <div className="allocation-page-container">
          ...
          <div className="selector-group">
            ...
            <Select
              ...
              value={studentOptions.filter((option) =>
                selectedStudentIds.includes(option.value)
              )}
      ...
      <div className="allocation-table-section">
        ...
        {allocationsError && (
          <p className="error-message">{allocationsError}</p>
        )}
            ...
            <tbody>
              {allocations.length === 0 ? (
                ...
                allocations.map((allocation) => {
                  ...
                  const displayStatus =
                    allocation.status === "active" && isOverdue
                      ? "overdue"
                  ...
                  return (
                    <tr key={allocation._id}>
                      ...
                      <td>
                        {allocation.durationMonths
                          ? `${allocation.durationMonths} months`
                          : "N/A"}
                      </td>
                      ...
                      <td>
                        ...
                        {allocation.status === "active" &&
                          !isOverdue && ( // Có thể ẩn nút hủy nếu quá hạn
                            <button
