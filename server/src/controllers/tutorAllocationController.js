...
exports.createAllocation = async (req, res) => {
  ...
  if (!Array.isArray(studentIds) || studentIds.length === 0) {
    return res
      ...
      .json({
        message:
          "Vui lòng cung cấp danh sách ID Sinh viên hợp lệ (studentIds).",
      });
  }
  if (!tutorId || !mongoose.Types.ObjectId.isValid(tutorId)) {
    return res
      .status(400)
      .json({ message: "Vui lòng cung cấp Tutor ID hợp lệ." });
  }
  ...
  if (
    isNaN(durationValue) ||
    durationValue <= 0 /* || ![1, 2, 3].includes(durationValue) */
  ) {
    return res
      .status(400)
      .json({
        message: "Vui lòng cung cấp thời gian khóa học hợp lệ (số tháng > 0).",
      });
  }
  ...
  try {
    ...
    if (!tutor) {
      return res
        .status(404)
        .json({ message: `Không tìm thấy Tutor với ID: ${tutorId}` });
    }
  } catch (err) {
  ...
  try {
    for (const studentId of studentIds) {
      if (!mongoose.Types.ObjectId.isValid(studentId)) {
        errors.push({ studentId, message: "ID không hợp lệ." });
        continue;
      }
      if (studentId === tutorId) {
        errors.push({
          studentId,
          message: "Không thể phân bổ gia sư cho chính họ.",
        });
        continue;
      }
      const student = await User.findOne({ _id: studentId, role: "student" });
      if (!student) {
        errors.push({ studentId, message: "Không tìm thấy Student." });
        continue;
      }
      ...
      try {
        ...
        const populated = await savedAllocation
          .populate([
        ...
        console.log(
          `Phân bổ thành công student ${studentId} cho tutor ${tutorId} với thời hạn ${durationValue} tháng`
        );
      } catch (allocError) {
        console.error(
          `Lỗi xử lý phân bổ cho student ${studentId}:`,
          allocError
        );
        errors.push({
          studentId,
          message: allocError.message || "Lỗi khi xử lý phân bổ.",
        });
      }
    }
    ...
    if (errors.length > 0 && results.length === 0) {
      return res
        .status(400)
        .json({ message: "Phân bổ thất bại cho tất cả sinh viên.", errors });
    } else if (errors.length > 0) {
      return res
        .status(207)
        .json({
          message: "Phân bổ hoàn tất với một số lỗi.",
          successfulAllocations: results,
          failedAllocations: errors,
        });
    } else {
      return res
        .status(201)
        .json({
          message: `Phân bổ thành công cho ${results.length} sinh viên.`,
          allocations: results,
        });
    }
  } catch (error) {
    ...
    res
      .status(500)
      .json({
        message: "Lỗi server khi thực hiện phân bổ.",
        error: error.message,
      });
  }
};
...
exports.getAllocations = async (req, res) => {
  try {
    ...
    if (req.query.status) {
      if (["active", "inactive"].includes(req.query.status)) {
        ...
        return res
          ...
          .json({
            message:
              "Giá trị status không hợp lệ (chỉ chấp nhận 'active' hoặc 'inactive').",
          });
      }
    }
    ...
    if (
      req.query.tutorId &&
      mongoose.Types.ObjectId.isValid(req.query.tutorId)
    ) {
    ...
    res
      .status(500)
      .json({
        message: "Lỗi server khi lấy allocations.",
        error: error.message,
      });
  }
};
...
exports.deactivateAllocation = async (req, res) => {
  ...
  try {
    const allocation = await Allocation.findByIdAndUpdate(
      ...
      .populate("tutor", "fullName"); // Populate nếu muốn trả về thông tin

    if (!allocation) {
      return res
        .status(404)
        .json({ message: `Không tìm thấy allocation với ID: ${id}` });
    }

    res
      .status(200)
      .json({ message: "Allocation đã được hủy kích hoạt.", allocation });
  } catch (error) {
    console.error("Lỗi hủy kích hoạt allocation:", error);
    res
      .status(500)
      .json({
        message: "Lỗi server khi hủy kích hoạt allocation.",
        error: error.message,
      });
  }
};
...
exports.getMyAllocations = async (req, res) => {
  try {
    ...
    if (userRole === "student") {
      ...
      console.log(
        `User role ${userRole} không phải student/tutor, không có 'my allocations'.`
      );
    ...
    res
      .status(500)
      .json({ message: "Lỗi server khi lấy thông tin phân công của bạn." });
  }
};
